...
export const MODELS_DIR = resolve(import.meta.dirname, "../models");

/**
 * @typedef {object} ModelResult
 * @property {string} file - submission filename in models/
 * @property {string} model - display name, from the submission's meta.model
 * @property {string} output - minified HTML; empty string when the submission failed
 ...
 */
export async function runModels(input) {
  ...
  for (const file of files) {
    /** @type {ModelResult} */
    const result = { file, model: file.replace(/\.js$/, ""), output: "" };
    try {
...
export function winner(results) {
  ...
  if (ok.length === 0) {
    throw new Error("every submission failed; run `npm run compare` for details");
  }
